  ...
  const act = async (app: string, restore: boolean) => {
    ...
    try {
      ...
      if (restore) {
        ...
        onSuccess(`已还原 ${APP_LABEL[app]} 的原始配置`);
      } else {
        await invoke("apply_takeover", { appType: appEnum });
        onSuccess(
          `${APP_LABEL[app]} 已接管,请求将经 Conduit 代理转发(免重启)`,
        );
      }
      refresh();
    } catch (e) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[460px]">
        <DialogHeader>
          <DialogTitle>代理接管</DialogTitle>
          <DialogDescription>
            接管会把 CLI 的配置指向本地代理(127.0.0.1:9527),切换供应商立即生效、
            无需重启终端。原始配置自动备份,可随时还原。
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  >
                    {s.active && s.effective && (
                      <>
                        <ShieldCheck className="w-3 h-3" /> 已接管,流量走代理
                      </>
                    )}
                    {s.active && !s.effective && (
                      <>
                    ...
                    {!s.active && (
                      <>
                        ...
                        {s.config_exists ? "未接管" : "未检测到配置(接管时将创建)"}
                      </>
                    )}
                  </div>
                </div>
